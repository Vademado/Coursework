...
void send_fuzz_data(u8* buf, s32 len)
{
  int64_t number = 0;
  u8 tmp[8];
  for (u8 i=0; i < len; ++i) tmp[i] = buf[i];
  for (u8 i=len; i < 8; ++i) tmp[i] = 0;
  u8 command[200];
  command[199] = 0;
  FILE* pf = fopen("./tmp/fuzz_tmp.bin", "wb"); 
  fwrite(tmp, 1, 8, pf);
  fclose(pf);
  sprintf(command, "python3.11 ../../../CFGGen/main.py -r=../../../CFGGen/cfg -inp=%lld -out=tmp", *((int64_t*)tmp));
  pf = fopen("./tmp/fuzz_data.bin", "wb");
  fwrite(command, 1, 200, pf);
  fclose(pf);
  system(command);
}
